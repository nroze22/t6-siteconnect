...
ROOT=Path(__file__).resolve().parents[1];profile=json.loads((ROOT/'src/lib/extraction/profile.json').read_text());cases=json.loads((ROOT/'sample-data/extraction/benchmark-cases.json').read_text());out=ROOT/'work/source-benchmark.json';results=[]
for case in cases:
 body={'model':'gemma4:e2b','stream':False,'think':profile.get('think',False),'keep_alive':profile['keep_alive'],'options':profile['options'],'format':profile['schema'],'messages':[{'role':'system','content':profile['system']+'\nRequired JSON schema: '+json.dumps(profile['schema'],separators=(',',':'))},{'role':'user','content':json.dumps(case['segments'],separators=(',',':'))}]}
 ...
 try:
  ...
  parsed=json.loads(response['message']['content']);entities=parsed['entities']
  errors=[]
  ...
  if errors:
   body['messages'] += [{'role':'assistant','content':json.dumps(parsed,separators=(',',':'))},{'role':'user','content':profile['repair_instruction']+json.dumps(errors,separators=(',',':'))}]
   request=urllib.request.Request('http://127.0.0.1:11434/api/chat',data=json.dumps(body).encode(),headers={'Content-Type':'application/json'})
   with urllib.request.urlopen(request,timeout=180) as r:response=json.load(r)
   parsed=json.loads(response['message']['content']);entities=parsed['entities'];attempts=2
  supported=[]
  for e in entities:
  ...
  for expected in case['expected']:
   matched.append(any(g and e['kind']==expected['kind'] and e['label'].lower()==expected['label'].lower() and e['value']==expected['value'] and e['unit']==expected['unit'] and e['assertion']==expected['assertion'] for e,g in zip(entities,supported)))
  passed=all(matched) and len(entities)==len(case['expected']) and all(supported) and response.get('done') and response.get('done_reason')!='length'
 ...
 results.append(row);print(json.dumps(row),flush=True);out.parent.mkdir(exist_ok=True);out.write_text(json.dumps({'model':body['model'],'profile':profile,'scope':'Ten short synthetic cases; exact field/assertion and verbatim evidence checks. Not clinical validation.','runs':results},indent=2))
