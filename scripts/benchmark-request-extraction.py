...
for case in cases:
 ...
 try:
  ...
  passed=all(matched) and len(entities)==len(case['expected']) and all(supported) and all(e.get('field') in profile['schema']['properties']['entities']['items']['properties']['field']['enum'] for e in entities) and response.get('done') and response.get('done_reason')!='length'
