...
const Candidate=z.object({segment_id:z.string(),kind:z.enum(['lab','medication','diagnosis','vital','demographic']),label:z.string().min(1).max(500),value:z.string().min(1).max(1000),unit:z.string().max(100).nullable(),subject:z.string().max(100).nullable(),assertion:z.enum(['present','negated','historical','uncertain','unknown']),quote:z.string().min(1).max(2500)}).strict();
export function validateEntities(raw:unknown,segments:Segment[]):Entity[]{
 ...
 return result.entities.map((e,i)=>{
  ...
  const supported=diagnosisValid&&unique&&e.quote.includes(e.label)&&e.quote.includes(e.value)&&(e.unit===null||(e.unit.length>0&&e.quote.includes(e.unit)))&&(e.subject===null||(e.subject.length>0&&e.quote.includes(e.subject)));
  return {...e,id:`${e.segment_id}-${i}`,origin:'model',start,end:start+e.quote.length,supported,problem:!diagnosisValid?'Diagnosis value must be the condition phrase; assertion carries uncertainty or negation':!s?'Unknown source segment':start<0?'Quote was not found exactly in the source':!unique?'Quote occurs more than once; source location is ambiguous':!supported?'A field is not present in the quoted passage':undefined};
 });
}
