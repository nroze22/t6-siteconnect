...
import {evaluateExtraction} from './evaluation';
import {noteGold} from './note-gold';
...
it.skipIf(process.env.SITECONNECT_LIVE_RECALL!=='1')('benchmarks actual default Gemma plus source recovery',async()=>{
 ...
 reports.push({...(name==='built-in-note'?{strictLabEvaluation:{scope:'Lab fields, patient/assertion and exact annotated evidence spans; excludes context candidates',model:evaluateExtraction(noteGold,model.filter(e=>e.kind==='lab'),segments),combined:evaluateExtraction(noteGold,combined.filter(e=>e.kind==='lab'),segments)}}:{}),name,model:'gemma4:e2b',profile:profile.version,recallProfile:RECALL_VERSION,seconds:(Date.now()-started)/1000,expectedValues:expected,modelMatched:found(model),combinedMatched:found(combined),ruleCandidates:rules,modelCandidates:model,combinedCandidates:combined,uncoveredMeasurements:uncoveredMeasurements(segments,combined)});
