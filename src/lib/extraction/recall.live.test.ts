...
it.skipIf(process.env.SITECONNECT_LIVE_RECALL!=='1')('benchmarks actual default Gemma plus source recovery',async()=>{
 ...
 const messages=[{role:'system',content:profile.system+'\nRequired JSON schema: '+JSON.stringify(profile.schema)+'\nRequest: '+JSON.stringify(REQUEST)},{role:'user',content:JSON.stringify(segments.map(({id,text})=>({id,text})))}];
 let model:ReturnType<typeof validateEntities>=[];
 for(let attempt=0;attempt<2;attempt++){
 const response=await fetch('http://127.0.0.1:11434/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'gemma4:e2b',stream:false,think:profile.think,keep_alive:profile.keep_alive,options:profile.options,format:profile.schema,messages}),signal:AbortSignal.timeout(180000)});
 if(!response.ok)throw Error(await response.text());const payload=await response.json();expect(payload.done_reason).not.toBe('length');model=validateEntities(JSON.parse(payload.message.content),segments);
