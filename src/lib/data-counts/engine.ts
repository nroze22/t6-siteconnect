...
import requestDefinition from './request.json';
export const REQUEST = requestDefinition;
...
export function sourceObservation(l:Lab){
 return {resourceType:'Observation',id:l.id,meta:{versionId:l.sourceVersion,tag:[{system:'urn:talosix:demo',code:'synthetic'}]},status:l.status,
  ...
  code:{coding:[{system:l.system,code:l.code,display:l.display,...(l.codingVersion!==undefined?{version:l.codingVersion}:{}),...(l.codingUserSelected!==undefined?{userSelected:l.codingUserSelected}:{})}],...(l.codingText!==undefined?{text:l.codingText}:{})},subject:{reference:`Patient/${l.patient}`},specimen:{display:l.specimen},
  effectiveDateTime:l.effectiveDateTime,issued:l.issued,
  ...(l.value!==null?{valueQuantity:{value:l.value,...(l.comparator?{comparator:l.comparator}:{}),...(l.unit?{unit:l.unit,system:l.unitSystem}:{}),...(l.quantityCode!==undefined?{code:l.quantityCode}:{}),...(l.unitSystem?{system:l.unitSystem}:{})}}:{dataAbsentReason:{coding:[{system:'http://terminology.hl7.org/CodeSystem/data-absent-reason',code:l.dataAbsentReason}]}}),...(l.referenceRange?{referenceRange:[{low:{value:l.referenceRange.low,unit:l.referenceRange.unit,...(l.referenceContext?.lowSystem?{system:l.referenceContext.lowSystem}:{}),...(l.referenceContext?.lowCode?{code:l.referenceContext.lowCode}:{})},high:{value:l.referenceRange.high,unit:l.referenceRange.unit,...(l.referenceContext?.highSystem?{system:l.referenceContext.highSystem}:{}),...(l.referenceContext?.highCode?{code:l.referenceContext.highCode}:{})},...(l.referenceContext?.text!==undefined?{text:l.referenceContext.text}:{})}]}:{})};
}
