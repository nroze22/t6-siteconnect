...
function Slider({
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-xs text-dim w-40 shrink-0">{label}</span>
      <input
        type="range"
        min={min}
        max={max}
  ...
  return (
    <motion.div variants={fadeUp} className="bg-card border border-edge-2 rounded-xl p-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-body">Research Readiness Score</h2>
          <p className="text-xs text-dim">Composite assessment of your site's clinical research potential</p>
        </div>
      </div>
  ...
  return (
    <motion.div variants={fadeUp} className="bg-card border border-edge-2 rounded-xl p-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-body">Missed Opportunity Detector</h2>
        ...
        {alerts.map((alert) => {
          ...
          return (
            <motion.div
              key={alert.id}
              layout
              className={`rounded-lg border p-4 ${styles.bg} ${styles.border} cursor-pointer transition-colors hover:bg-surface-2`}
              onClick={() => setExpanded(isExpanded ? null : alert.id)}
            >
              <div className="flex items-start gap-3">
                <div className="mt-0.5 text-dim">{alert.icon}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span
                      className={`text-[12px] font-semibold uppercase px-1.5 py-0.5 rounded ${styles.badge}`}
                    >
                      {alert.severity}
                    </span>
                  </div>
                  <h3 className="text-sm font-medium text-body leading-snug">
                    {alert.title}
                  </h3>
                  {isExpanded && (
                    <motion.div
  ...
  return (
    <motion.div variants={fadeUp} className="bg-card border border-edge-2 rounded-xl p-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-body">Enrollment Yield Funnel</h2>
          <p className="text-xs text-dim">
        ...
        <button
          ...
          className="ml-auto flex items-center gap-1.5 text-xs text-dim hover:text-body transition-colors px-2 py-1 rounded-md hover:bg-surface-2"
        >
      ...
      {showSliders && (
        <motion.div
          ...
          animate={{ opacity: 1, height: "auto" }}
          exit={{ opacity: 0, height: 0 }}
          className="mb-6 bg-surface-1 rounded-lg p-4 space-y-3 border border-edge-1"
        >
            ...
            <div key={stage.label} className="relative">
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  ...
                  <span className="relative text-sm font-bold text-heading ml-4 drop-shadow-sm">
                    <AnimatedCount value={stage.count} />
                  </span>
                </div>
                <div className="shrink-0">
                  <span className="text-xs text-body font-medium">{stage.label}</span>
                </div>
              </div>
            </div>
          );
      ...
      {/* Summary */}
      <div className="mt-6 flex items-center gap-6 text-xs text-dim border-t border-edge-1 pt-4">
        <div>
          <span className="text-dim">Overall Yield:</span>{" "}
        ...
        <div>
          <span className="text-dim">Projected Randomized:</span>{" "}
      ...
      <div className="mt-6 border-t border-edge-1 pt-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-semibold text-body">Monte Carlo Enrollment Forecast</h3>
        ...
        {mcResult !== null && (
          ...
          >
            <div className="bg-surface-1 border border-edge-1 rounded-lg p-3">
  ...
  return (
    <motion.div variants={fadeUp} className="bg-card border border-edge-2 rounded-xl p-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-body">Study Activation ROI Calculator</h2>
          <p className="text-xs text-dim">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          {/* Per-Subject Value */}
          <div>
            <label className="flex items-center justify-between text-xs text-dim mb-1">
              <span>Per-Subject Value</span>
              <span className="font-mono text-body">${formatNumber(inputs.perSubjectValue)}</span>
            </label>
            <input
              type="range"
              min={5000}
          ...
          <div>
            <label className="flex items-center justify-between text-xs text-dim mb-1">
              ...
              <span className="font-mono text-body">{inputs.eligibleSubjects}</span>
            </label>
            <input
              type="range"
              min={1}
          ...
          <div>
            <label className="flex items-center justify-between text-xs text-dim mb-1">
              ...
              <span className="font-mono text-body">${formatNumber(inputs.activationCost)}</span>
            </label>
            <input
              type="range"
              min={5000}
          ...
          <div>
            <label className="flex items-center justify-between text-xs text-dim mb-1">
              ...
              <span className="font-mono text-body">${formatNumber(inputs.coordinatorCost)}</span>
            </label>
            <input
              type="range"
              min={30000}
          ...
          <div>
            <label className="flex items-center justify-between text-xs text-dim mb-1">
              ...
              <span className="font-mono text-body">{inputs.durationMonths} months</span>
            </label>
            <input
              type="range"
              min={6}
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4 text-xs">
            <div className="flex-1 bg-surface-1 border border-edge-1 rounded-lg px-3 py-2">
              <div className="text-[12px] text-dim mb-0.5">ROI</div>
              <div
            ...
            <div className="flex-1 bg-surface-1 border border-edge-1 rounded-lg px-3 py-2">
              <div className="text-[12px] text-dim mb-0.5">Payback Period</div>
              <div className="text-sm font-bold font-mono text-body">
                {roi.paybackMonths} mo
              </div>
            </div>
            <div className="flex-1 bg-surface-1 border border-edge-1 rounded-lg px-3 py-2">
              <div className="text-[12px] text-dim mb-0.5">Rev / Coord Month</div>
              <div className="text-sm font-bold font-mono text-body">
          ...
          {/* Break-even chart */}
          <div>
            <h4 className="text-xs text-dim mb-3">Break-Even Analysis</h4>
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="flex items-center gap-4 mt-2 text-[12px] text-dim">
  ...
  if (loading) {
    return (
      <div className="p-6 space-y-6">
        <div className="mb-2">
          <div className="h-7 w-56 bg-surface-2 rounded animate-pulse" />
          <div className="h-4 w-96 bg-surface-2 rounded animate-pulse mt-2" />
        </div>
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="h-64 bg-card border border-edge-2 rounded-xl animate-pulse"
          />
        ))}
      </div>
    );
