...
export function DrillDownPanel() {
  ...
  function SortIcon({ column }: { column: SortKey }) {
    if (!sort || sort.key !== column) {
      return <ArrowUpDown className="h-3 w-3 text-dim" />;
    }
    return sort.dir === "asc" ? (
      ...
      {drillDown && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 z-50 flex h-full w-[520px] flex-col bg-card shadow-2xl border-l border-edge-2"
          >
            {/* ---- Header ---- */}
            <div className="flex-shrink-0 border-b border-edge-2 px-6 py-5">
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <h2 className="text-lg font-semibold text-body truncate">
                    {drillDown.title}
                  </h2>
                ...
                <button
                  onClick={closeDrillDown}
                  className="ml-4 flex-shrink-0 rounded-lg p-1.5 text-dim hover:bg-surface-3 hover:text-body transition-colors"
            ...
            {/* ---- Export Bar ---- */}
            <div className="flex-shrink-0 flex items-center gap-2 border-b border-edge-2 px-6 py-3">
              <button
                onClick={handleExportCSV}
                disabled={patients.length === 0}
                className="inline-flex items-center gap-1.5 rounded-md bg-surface-3 px-3 py-1.5 text-xs font-medium text-body hover:bg-surface-5 hover:text-slate-100 transition-colors disabled:opacity-40 disabled:pointer-events-none"
              >
            ...
            {/* ---- Summary Stats ---- */}
            {patients.length > 0 && (
              <div className="flex-shrink-0 grid grid-cols-3 gap-3 border-b border-edge-2 px-6 py-4">
                ...
                <div className="rounded-lg bg-card px-3 py-2">
                  ...
                  <div className="text-xs font-medium text-body truncate" title={stats.topDx}>
                    {stats.topDx}
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-hidden flex flex-col min-h-0">
              {/* Table header */}
              <div className="flex-shrink-0 flex items-center gap-0 border-b border-edge-2 px-6 py-2">
                {COLUMNS.map((col) => (
                  <button
                    key={col.key}
                    onClick={() => handleSort(col.key)}
                    className={`flex items-center gap-1 text-[12px] uppercase tracking-wider font-medium text-dim hover:text-body transition-colors ${col.width} truncate`}
                  >
                    {col.label}
              ...
              <div className="flex-1 overflow-y-auto">
                {sortedPatients.length === 0 ? (
                  ...
                  sortedPatients.map((p) => (
                    <div
                      key={p.mrn}
                      className="flex items-center gap-0 border-b border-edge-1 px-6 py-2.5 hover:bg-surface-2 transition-colors"
                    >
                      <div className={`${COLUMNS[0]!.width} text-xs font-mono text-dim truncate`}>
                        {p.mrn}
                      </div>
                      <div className={`${COLUMNS[1]!.width} text-xs text-body truncate`}>
                        {p.lastName}, {p.firstName}
                      </div>
                      <div className={`${COLUMNS[2]!.width} text-xs text-dim tabular-nums`}>
                        {calculateAge(p.dob)}
                      </div>
                      <div className={`${COLUMNS[3]!.width} text-xs text-dim`}>
                        {p.sex.charAt(0)}
                      </div>
                      <div className={`${COLUMNS[4]!.width} text-xs text-body truncate`} title={primaryDiagnosis(p)}>
                        {primaryDiagnosis(p)}
                      </div>
                      <div className={`${COLUMNS[5]!.width} text-xs text-dim truncate`} title={keyLabs(p)}>
                        {keyLabs(p)}
                      </div>
                    </div>
                  ))
