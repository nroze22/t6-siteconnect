...
it('retains reviewed source when replacement is cancelled or unreadable',async()=>{
 ...
 fireEvent.click(screen.getByText(/Change file or reader settings/));fireEvent.click(screen.getByRole('button',{name:'Load example note'}));expect(screen.getByRole('dialog')).toBeInTheDocument();fireEvent.click(screen.getByRole('button',{name:'Keep reviewing'}));expect(readSource).toHaveBeenCalledTimes(1);
 vi.mocked(readSource).mockRejectedValueOnce(Error('Invalid file'));
 fireEvent.click(screen.getByText(/Change file or reader settings/));fireEvent.click(screen.getByRole('button',{name:'Load example note'}));fireEvent.click(screen.getByRole('button',{name:'Replace document'}));await screen.findByRole('alert');expect(screen.getByText('kept.csv')).toBeInTheDocument();expect(screen.getByLabelText('I reviewed the parsed source fields and file coverage.')).toBeChecked();expect(screen.getByRole('button',{name:'Export review & open questions'})).toBeEnabled();
});
